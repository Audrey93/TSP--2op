...
public:
    ...
    vector<int> turaNajmanjaGrana();
    vector<int> ruta, novaRuta;
    bool TrosakBezGrana(int i,int j);
    void Run();
    void TwoOpt();
...
vector<int> Graf::turaNajmanjaGrana() {
    Ruta ruta(n);
...
void Graf::Run()
{
    ruta = turaNajmanjaGrana();
    cout << "\nDuzina optimalne ture dobijane primjenom metode Najmanja grana je: " <<duzinaTure(ruta);
    TwoOpt();
    cout << "\nDuzina optimalne ture dobijane primjenom metode 2op je: " << duzinaTure(ruta) << endl;
}

bool Graf::TrosakBezGrana(int i, int j){
    if( (udaljenosti[(n-1+i)%n][i] + udaljenosti[j][(j+1)%n]) > (udaljenosti[(n-1+i)%n][j] + udaljenosti[i][(j+1)&n]) )
        return true;
    return false;
}
...
{
    novaRuta = ruta;
    bool potrebnePromjene = true;
    double novaUdaljenost;
    double najUdaljenost = duzinaTure(ruta);

    while(potrebnePromjene)
    {
        potrebnePromjene = false;
        for ( int i = 0; i < n - 1; i++ ){
            for ( int k = i + 1; k < n; k++){
                //if(TrosakBezGrana(i,k)){
                TwoOptSwap(i, k);
                novaUdaljenost = duzinaTure(novaRuta);

               if(novaUdaljenost < najUdaljenost){

                    potrebnePromjene = true;
                    ruta = novaRuta;
                    najUdaljenost = novaUdaljenost;
...
void Graf::TwoOptSwap( const int& i, const int& k ){
for ( int j = 0; j < i; j++ )
{
novaRuta[j] = ruta[j];
}
